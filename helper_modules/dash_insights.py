...
def time_series_info(df):
    ...
    date_cols = temp_df.select_dtypes(include=['datetime64']).columns
    numeric_cols = temp_df.select_dtypes(include=['int64', 'float64']).columns
    ...
    if len(date_cols) > 0:
        ...
        # Grouping by Month
        for num_col in numeric_cols:
            if pd.api.types.is_datetime64_any_dtype(temp_df['Month']):
                temp_df['Month'] = temp_df['Month'].dt.month
            
            if temp_df['Month'].dtype != 'int64':  # Handle non-integer months
                try:
                    temp_df['Month'] = temp_df['Month'].astype(int)
                except:
                    raise ValueError("Month column could not be converted to integer.")
            group_means = temp_df.groupby('Month')[num_col].mean()
    ...
    # Grouping by Year
        for num_col in numeric_cols:
            if pd.api.types.is_datetime64_any_dtype(temp_df['Year']):
                temp_df['Year'] = temp_df['Year'].dt.year
            group_means = temp_df.groupby('Year')[num_col].mean()
...
# print("\n")
# print(numerical_insights(df))
# print("\n")
# print("----- Categorical insights ------")
# print("\n")
# print(categorical_numerical_info(df))
